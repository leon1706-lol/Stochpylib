...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-1756%20passing-brightgreen?style=flat-square&labelColor=1A1A1A" alt="1756 of 1758 tests passing">
  ...
  <img src="https://img.shields.io/badge/public%20names-569%20of%20794-FF8C00?style=flat-square&labelColor=1A1A1A" alt="569 of 794 spec names implemented">
</p>
...
confidence interval. Around that contract, sixteen modules are live today: a **probability
...
with Bayes factors). The roadmap takes
it onward through nonparametric methods and beyond (23 modules, ~794 public names planned). The
...
- **PyPI lags the repository.** The latest published release is `0.1.1`; the code here is at
  `0.13.0`. Releases are tag-triggered (see [Release Process](#release-process)) and no tag has
...
- **7 of 23 planned modules remain.** The implemented sixteen are complete and tested against
...
spl --test                  # embedded self-check (199 checks), no pytest needed
```
...
Sixteen modules implemented and tested — **569 / 794 spec names**:
...
(currently **569 / 794 public names**).
...
|---|---|---|
| `stochpylib/` | [package guide](stochpylib/README.md) | the installable package: sixteen module subpackages, `cli.py`, `selftest.py` |
...
**1756 passed / 2 skipped** as of the V0.13.0 `bayesian` implementation (the 2 permanent skips
...
Runs the embedded self-check suite shipped inside the wheel (**199 checks**): package sanity and per-module spec
...
a sprinkler-network Bayesian-network query). Bare `spl demo` lists the available demos.
...
Seven modules remain on the spec (in rough implementation order): nonparametric methods,
robust statistics, spatial statistics, optimization, experimental design, visualization,
and utilities.
