...
- `tests/<module>/tests.py` — one oracle suite per implemented module (sixteen
...
- `tests/library/tests.py` — the cross-module suite: spec-name conformance for
  all 569 implemented public names (generated from
  ...
  `Posterior`/`PosteriorApproximation`/`ICResult`/`EmpiricalPredictive`), the
  sanctioned multivariate method-contract deviation, and end-to-end workflows
  spanning modules (reliability MC on library Weibull, t-copula margins through
  the library Student_t, ARIMA vs GP forecasting agreement, `CopulaFit` refit
  round trips, Sobol-QMC vs crude consistency, a conjugate Bayesian posterior
  cross-checked against `statistics.bayesian_estimator` and an MCMC sampler).
...
`spl --test` (199 checks), which includes the per-module conformance and
