...
- `tests/<module>/tests.py` — one oracle suite per implemented module (fifteen
...
- `tests/library/tests.py` — the cross-module suite: spec-name conformance for
  all 544 implemented public names (generated from
  ...
  `SDESolution`/`Mesh`), the sanctioned multivariate method-contract deviation, and
...
`spl --test` (187 checks), which includes the per-module conformance and
