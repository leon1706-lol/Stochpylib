...
- `tests/<module>/tests.py` — one oracle suite per implemented module (seventeen
  ...
  numerical_methods, bayesian, robust_statistics).
...
- `tests/library/tests.py` — the cross-module suite: spec-name conformance for
  all 597 implemented public names (generated from
  ...
  `Resampler`/`SiegelRegression`), the sanctioned multivariate method-contract
...
`spl --test` (212 checks), which includes the per-module conformance and
